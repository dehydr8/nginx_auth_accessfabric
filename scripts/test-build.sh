...
./configure --add-dynamic-module="${DIR}/../" --prefix="${NGINX_INST_DIR}" --with-cc-opt="-I${LIBXJWT_INST_DIR}/usr/local/include" --with-ld-opt="-L${LIBXJWT_INST_DIR}/usr/local/lib -Wl,rpath=${LIBXJWT_INST_DIR}/usr/local/lib"
make
make install
